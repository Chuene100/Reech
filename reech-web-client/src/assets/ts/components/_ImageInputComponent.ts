...
class ImageInputComponent {
  ...
  private _change = (e: Event) => {
    ...
    if (this.inputElement !== null && this.inputElement.files && this.inputElement.files[0]) {
      // Fire change event
      if (EventHandlerUtil.trigger(this.element, 'reech.imageinput.change', e) === false) {
        return
      }
      ...
      // Fire removed event
      EventHandlerUtil.trigger(this.element, 'reech.imageinput.changed', e)
    }
  }
  ...
  private _cancel = (e: Event) => {
    ...
    // Fire cancel event
    if (EventHandlerUtil.trigger(this.element, 'reech.imageinput.cancel', e) === false) {
      return
    }
    ...
    // Fire canceled event
    EventHandlerUtil.trigger(this.element, 'reech.imageinput.canceled', e)
  }
  ...
  private _remove = (e: Event) => {
    ...
    // Fire remove event
    if (EventHandlerUtil.trigger(this.element, 'reech.imageinput.remove', e) === false) {
      return
    }
    ...
    // Fire removed event
    EventHandlerUtil.trigger(this.element, 'reech.imageinput.removed', e)
  }
