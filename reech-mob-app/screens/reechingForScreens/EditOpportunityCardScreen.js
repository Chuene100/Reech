...
import { useUpdateOpportunityMutation } from "../../redux/api/opportunity";
...
const EditOpportunityCardScreen = ({ route }) => {
  const navigation = useNavigation();
  const { opp } = route.params;
  ...
  const [monthSelector, setMonthSelector] = useState(1);
  ...
  const [uploadFn, { isLoading: isUploadingImage }] =
    useUploadSingleFileMutation();
  const [updateFn, { isLoading }] = useUpdateOpportunityMutation();
  ...
  React.useEffect(() => {
    const start_date = moment(opp?.startDate).format("D-M-YYYY").split("-");
    const exp = isNaN(opp?.experience) ? 0 : Number(opp?.experience);
    const rad = opp?.radius / 1000;

    setIncognitoActive(opp?.postIncognito);
    setBusinessesActive(opp?.businessOnly);
    setBubbleActive(opp?.bubbleMatesOnly);
    setPersonalActive(opp?.postPersonal);
    setDaySelector(Number(start_date[0]));
    setMonthSelector(Number(start_date[1]));
    setYearSelector(Number(start_date[2]));
    setApplicantSelector(opp?.maxApplicants);

    setValue("whoIam", opp?.description?.whoIam);
    setValue("whereFitIn", opp?.description?.whereFitIn);
    setValue("helpYouSucceed", opp?.description?.helpYouSucceed);
    setValue("rateFrequency", opp?.rateFrequency);
    setValue("rateCurrency", opp?.rateCurrency);
    setValue("rate", opp?.rate);
    setValue("duration", opp?.duration);
    setValue("radius", rad);
  ...
  const onUpdatePressed = async (_data) => {
    ...
    const formattedDate = moment(_startDate, "D MM YYYY").format("YYYY-MM-DD");

    const payload = {
      skills: [
        {
          skillId: "638a7bef89d9d75be43aac2b",
      ...
      jobCategoryId: "6454a4fe31d8db9c05608ea5",
      address: loc?.[0],
      location: {
        type: "Point",
        coordinates: loc ? [Number(loc[2]), Number(loc[1])] : undefined, //[longitude, latitude]
      },
      radius: _data?.radius * 1000,
      experience: _data?.experience,
      NQFLevel: _data?.educationLevel
        ? Number(_data?.educationLevel?.split("|")[1])
        : undefined,
      educationLevel: _data?.educationLevel?.split("|")?.[0],
      ...
      jobDescription:
        _data.whoIam + ", " + _data.whereFitIn + ", " + _data.helpYouSucceed,
    ...
    if (_data?.oppImage !== opp?.oppImage) {
    ...
    updateFn({ body: payload, oppId: opp?._id })
      .then(async (res) => {
        if (res.error) {
          showError(res);
          return;
        }
        showToast("Your opportunity has been updated!");
        navigation.goBack();
      })
      .catch((err) => {
  ...
  function renderHeaderSection() {
    return (
      <View style={styles.contentContainerEditOpp}>
        <NavHeader
  ...
  function renderHeadingTopSection() {
    return (
      <View style={styles.headingContentContainer}>
        ...
        <Text style={styles.headingTextItem}>Update an opportunity card</Text>
      </View>
    );
  }
      ...
      <View style={styles.opportunityLocationContainer}>
        ...
        <View style={styles.searchComponentContainer}>
          ...
          <CustomReechForLocation
            name="address"
            control={control}
            placeholder={opp?.address ?? "State, Country"}
          />
      ...
      <View style={styles.opportunityEducationContainer}>
        ...
        <View style={styles.opportunityEducationDropDownContainer}>
          <DropDown
            ...
            name="educationLevel"
            placeholder={opp?.educationLevel}
            control={control}
            data={qualification}
            textAlign={'center'}
            iconColor={'purple'}
      ...
      <View style={styles.opportunityDateContainer}>
        ...
        <View style={styles.OpportunityDateComponentContainer}>
          ...
          <View style={styles.opportunityYearItemContainer}>
            ...
            <TouchableOpacity onPress={() => setYearSelector(Math.min(currYear + 5, yearSelector + 1))}>
              <Entypo
      ...
      <View style={styles.opportunityTypeContainer}>
        ...
        <View style={styles.opportunityTypeDropDownContainer}>
          <DropDown
            ...
            placeholder={opp?.opportunityType}
            control={control}
            textAlign={'center'}
            iconColor={'purple'}
          />
      ...
      <View style={styles.opportunityImageSectionContainer}>
        ...
        {libraryUserPicker && (
          <View style={styles.imagePickerComponentContainer}>
            <CustomImageOppPicker name="oppImage" control={control} />
          </View>
        )}
      </View>
    );
  ...
  function renderOpportunityCardPresentationSection() {
    return (
      <View style={styles.opportunityImageLookSectionContainer}>
        ...
        <View style={styles.careersOpportunityContainer}>
          <ImageBackground
            source={
              libraryUserPicker
                ? typeof watchOppImage === "string" && libraryUserPicker
                  ? { uri: watchOppImage }
                  : { uri: images["c19"] }
                : { uri: images?.[opp?.oppImage] ?? opp?.oppImage }
            }
              ...
              >
                ...
                <View style={styles.opportunitysNameContainer}>
                  ...
                  <View style={styles.opportunitysNameContent}>
                    ...
                    >
                      {opp?.jobTitle ?? watch("jobTitle")}
                    </Text>
                  </View>

                  {/*opportunity info trigger*/}
                  <View style={styles.opportunitysInfoContent}>
                    <MaterialIcons
                      name="info"
                      size={25}
                      color={COLORS.white}
  ...
  function renderSubmitOpportunityActionSection() {
    return (
      <View style={styles.reechButtonContainer}>
        <TouchableOpacity onPress={handleSubmit(onUpdatePressed)}>
        ...
        <Text style={styles.reechTextItem}>Press button to update card</Text>
      </View>
    );
  }
...
export default EditOpportunityCardScreen;
