...
const AuthProfilePersonaliserScreen = ({ route }) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
  ...
  const [registerSeeker, { isLoading: isLoadingRegister }] =
  ...
  const [initDocFn, { isLoading: isLoadingInitDoc }] =
  ...
  React.useEffect(() => {
    setJobDt(jobTitleData?.skillIds);
  }, [jobTitleData]);
    ...
    registerSeeker(info)
      .then((res) => {
        ...
        initDocFn({ _id: res?.data?.data?._id }).then((res) => {
          ...
          navigation.navigate("AuthRenderPersonalisationScreen", {
            data: data,
          });
        });
      })
  ...
  function renderNavigationSection() {
    return (
      <View style={styles.headerContainer}>
        <View style={styles.headerTopTextContainer}>
        ...
        >
          <View
            style={[
              ...
              {
                backgroundColor:
                  selected !== -1 ? COLORS.purple : COLORS.darkGray,
              },
            ]}
          >
...
export default AuthProfilePersonaliserScreen;
