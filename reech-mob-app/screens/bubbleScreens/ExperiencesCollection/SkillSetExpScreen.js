...
const SkillSetExpScreen = ({ Feed }) => {
  const navigation = useNavigation();
  const image = useSelector((state) => state.bubble_images.bubbleImages);

  const Data = Feed?.filter((feed) => feed.cardType === "My bloopers");
  const [skillSetDataCollection] = useState(Data);
...
export default SkillSetExpScreen;
