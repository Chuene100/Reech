...
const ExperienceTimelineExpScreen = ({ Feed }) => {
  const navigation = useNavigation();
  const image = useSelector((state) => state.bubble_images.bubbleImages);

  const Data = Feed?.filter((feed) => feed.cardType === "Thought");
  const [experienceTimeLineDataCollection] = useState(Data);
  ...
  function renderItemCollection() {
    return (
      <SafeAreaView style={styles.itemContainer}>
        <View>
          <FlatList
            data={experienceTimeLineDataCollection}
...
export default ExperienceTimelineExpScreen;
