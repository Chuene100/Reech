...
const EventsExpScreen = ({ Feed }) => {
  const navigation = useNavigation();
  const image = useSelector((state) => state.bubble_images.bubbleImages);

  const Data = Feed?.filter((feed) => feed.cardType === "Events & Conferences");
  const [eventsDataCollection] = useState(Data);
...
export default EventsExpScreen;
