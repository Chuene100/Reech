...
const WorkExpScreen = ({ Feed }) => {
  const navigation = useNavigation();
  const image = useSelector((state) => state.bubble_images.bubbleImages);

  const Data = Feed?.filter((feed) => feed.cardType === "Where I've worked");
  const [workExperienceDataCollection] = useState(Data);
  ...
  function renderItemCollection() {
    return (
      <SafeAreaView style={styles.itemContainer}>
        <View>
          <FlatList
            data={workExperienceDataCollection}
