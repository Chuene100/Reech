...
const QualificationExpScreen = ({ Feed }) => {
  const navigation = useNavigation();
  const image = useSelector((state) => state.bubble_images.bubbleImages);

  const QualificationData_ = Feed?.filter(
    (feed) => feed.cardType === "What qualifies me"
  );
  const [qualificationDataCollection] = useState(QualificationData_);
  ...
  function renderItemCollection() {
    return (
      <SafeAreaView style={styles.itemContainer}>
        <View>
          <FlatList
            data={qualificationDataCollection}
...
export default QualificationExpScreen;
